...
async def chat_bot(req: ChatRequest):
    if _LLM_BUNDLE is not None:
        # Generative AI Path
        import re
        system_prompt = "You are the Hustlr App Support Assistant. Keep answers concise, 1-2 sentences. " \
                        "Hustlr offers Parametric Insurance. Standard Shield is ₹49/week. Full Shield is ₹79/week. " \
                        "Payouts are automatic if sensors (Rain, Extreme Heat, AQI) trace above limits. " \
                        "Withdraw payouts to UPI directly from the wallet."
        ...
        prompt = _LLM_BUNDLE.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        # Generate the prose
        outputs = _LLM_BUNDLE(prompt, max_new_tokens=60, do_sample=True, temperature=0.7)
        full_text = outputs[0]["generated_text"]
        
        # Parse the output suffix specifically generated by TinyLlama's chat template
        reply = full_text.split("<|assistant|>")[-1].strip()
        
        return ChatResponse(intent="generative_ai", response=reply, confidence=1.0)

    # Fallback to Scikit-Learn TF-IDF model if HuggingFace isn't ready
    elif _CHATBOT_BUNDLE:
    ...
    return ChatResponse(
        intent="default", 
        response="I'm here to help! My central ML brain is currently installing logic updates.", 
        confidence=0.0
    )
