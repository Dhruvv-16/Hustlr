"""
ring_detector.py ΓÇö Statistical ring-fraud detection for Hustlr.
...
1. Poisson Distribution Test (timestamp analysis)
   ...
   20ΓÇô40 minutes following an event, forming a Poisson inter-arrival
   distribution.  Coordinated rings fire uniformly within seconds of
   each other.  We use the dispersion index D = Var(╬öt)/Mean(╬öt) and
...
2. DBSCAN Geographic Clustering (GPS analysis)
   ...
   We flag any cluster with ΓëÑ 5 workers within a 50m radius.
"""
...
# ΓöÇΓöÇ Haversine distance (meters) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
def _haversine_m(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """
    Return the great-circle distance in metres between two WGS-84 coordinates.

    Uses the haversine formula ΓÇö accurate to < 0.5% for distances < 1 km,
    which is the relevant scale for dark-store delivery zones.
    """
    R = 6_371_000.0  # Earth radius in metres
    ╧å1, ╧å2 = math.radians(lat1), math.radians(lat2)
    ╬ö╧å = math.radians(lat2 - lat1)
    ╬ö╬╗ = math.radians(lon2 - lon1)
    a = math.sin(╬ö╧å / 2) ** 2 + math.cos(╧å1) * math.cos(╧å2) * math.sin(╬ö╬╗ / 2) ** 2
    return R * 2 * math.asin(math.sqrt(a))


# ΓöÇΓöÇ Poisson inter-arrival test ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

RING_P_VALUE_THRESHOLD = 0.05   # p < 0.05 ΓåÆ reject Poisson ΓåÆ coordinated ring
BURST_WINDOW_SECONDS   = 30     # Γëñ 30s spread across all claims = "burst"
...
def test_poisson_arrivals(timestamps: List[int]) -> dict:
    ...
    Algorithm:
      1. Sort timestamps and compute inter-arrival gaps (╬öt).
      2. Compute the dispersion index D = Var(╬öt) / Mean(╬öt).
         D Γëê Mean(╬öt) for a Poisson process (memoryless).
         D Γë¬ Mean(╬öt) indicates uniform/burst arrivals (ring).
      3. Run a one-sample KS test of ╬öt against an Exponential distribution
         with ╬╗ = 1/Mean(╬öt) ΓÇö the null hypothesis is Poisson arrivals.
    ...
    if len(timestamps) < 3:
        return {
            ...
            "note": "Insufficient data (< 3 claims) ΓÇö cannot evaluate ring pattern",
        }
...
# ΓöÇΓöÇ DBSCAN GPS clustering ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

RING_MIN_CLUSTER_SIZE    = 5      # ΓëÑ 5 workers in a tight cluster = ring
...
def detect_gps_clusters(gps_coords: List[Tuple[float, float]]) -> dict:
    ...
    Algorithm:
      ...
      4. Flag if any cluster has ΓëÑ 5 workers within 50m.
...
# ΓöÇΓöÇ Combined ring verdict ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def combined_ring_verdict(poisson_result: dict, dbscan_result: dict) -> str:
    ...
    Decision matrix:
      Both positive  ΓåÆ human_review   (high confidence ring)
      Poisson only   ΓåÆ soft_hold      (temporal anomaly, await GPS confirm)
      DBSCAN only    ΓåÆ soft_hold      (spatial anomaly, may be dark-store queue)
      Neither        ΓåÆ auto_approve   (no ring signal)
    """
