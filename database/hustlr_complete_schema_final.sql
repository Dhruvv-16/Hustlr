...
-- Balance = SUM(amount) — credits positive, debits negative.
-- No separate wallets table. Balance calculated at query time.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND constraint_name = 'wallet_amount_sign_matches_type'
  ) THEN
    ALTER TABLE wallet_transactions
      ADD CONSTRAINT wallet_amount_sign_matches_type
      CHECK (
...
DECLARE
  ...
  tables TEXT[] := ARRAY[
    ...
    'weekly_settlements','notifications','referrals',
...
DECLARE
  ...
  tables_with_ts TEXT[] := ARRAY[
    'users','risk_pools','policies','claims'
  ];
BEGIN
...
DECLARE
  gap_sec INTEGER;
BEGIN
  SELECT EXTRACT(EPOCH FROM (p_resume_time - gap_start))::INTEGER
    INTO gap_sec
    FROM shift_gaps
...
-- 1) Replace old strict constraint with relaxed one
ALTER TABLE wallet_transactions
...
-- 2) Normalize only truly wrong-sign legacy rows
...
-- 3) Validate constraint after cleanup
ALTER TABLE wallet_transactions
  VALIDATE CONSTRAINT wallet_amount_sign_matches_type;
