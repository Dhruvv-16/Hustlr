...
const BANDH_KEYWORDS = ["bandh", "curfew", "strike", "section 144", "road blocked", "shutdown", "hartal", "protest blocked"];
const CITY_KEYWORDS = {
...
async function checkBandhNLP(zone = 'adyar dark store zone') {
  ...
  try {
    ...
    console.log(`[Bandh NLP] LIVE NewsAPI | zone=${zone} | confidence=${confidence.toFixed(2)}`);
    return {
      source: 'live_newsapi',
      zone: zone,
    ...
    console.warn(`[NewsAPI] failed: ${e.response?.data?.message || e.message} — trying Brave Search`);
  }
  ...
  try {
    if (BRAVE_KEY) {
      ...
      const combinedText = results.map(r => ((r.title || '') + ' ' + (r.description || ''))).join(' ').toLowerCase();
      
      const matched = BANDH_KEYWORDS.filter(kw => combinedText.includes(kw));
      const confidence = Math.min(matched.length / 3, 1.0);

      console.log(`[Bandh NLP] FALLBACK Brave | zone=${zone} | confidence=${confidence.toFixed(2)}`);
      return {
