'use strict';
/**
 * nlp_service.js — NLP M4 improvements per ML Blueprint v1.0
 * ===========================================================
 * Adds:
...
const SOURCE_WEIGHTS = {
  ...
  worker:  0.90,   // In-app worker reports (potential bias/sarcasm)
  unknown: 1.00,   // Default when source not specified
};
...
function checkCompoundConfirmation(
  ...
  if (m4Severity === 3 && m6Classification === 'ACCIDENT_BLACKSPOT' && within30Min) {
...
/**
 * classifyDisruptionText — high-level helper used by disruption_cron.js.
...
async function classifyDisruptionText(text, opts = {}) {
  ...
  try {
    ...
    const resp   = await fetch(`${mlUrl}/nlp`, {
      ...
      body:    JSON.stringify({ text, require_dual_source: requireDualSource, sources }),
    });
    ...
    const severity   = result.hourly_rate_inr >= 60 ? 3 : result.hourly_rate_inr >= 45 ? 2 : 1;
