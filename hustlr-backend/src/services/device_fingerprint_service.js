...
// Requires `device_fingerprint_events` from schema_phase2.sql
 ...
 */
async function getFingerprintStats({ zone = null, days = WINDOW_DAYS, limit = 30 } = {}) {
  const since = new Date(
  ...
  for (const row of data || []) {
    ...
    if (!byHash.has(h)) byHash.set(h, { users: new Set(), zones: new Set(), events: 0 });
