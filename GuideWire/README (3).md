<div align="center">
  ...
  <strong>👥 Team:</strong> [Code Crackers] &nbsp;|&nbsp; <strong>🎯 Persona:</strong> Q-Commerce Delivery Partners (Zepto / Blinkit)
</div>
...
**Who:** Q-commerce delivery riders (Zepto / Blinkit) — 2–3 km radius, one dark store, zero income safety net.
...
**How it's built:** Flutter app · Node.js + Supabase backend · 7 AI/ML models · 6-layer fraud engine · Full Guidewire integration (PolicyCenter + ClaimCenter + BillingCenter).

**Numbers:** ₹29–₹109/week · ₹300/day payout cap · 55–68% projected loss ratio · ₹0 infrastructure cost · 10,000-worker Chennai pilot.
...
4. Fixed payout credited instantly   →  ₹50/hr × verified disruption hours, capped at ₹150/day
```

<p align="center">
  <img src="https://github.com/user-attachments/assets/289c1d4b-ce38-4355-81ce-223381723260" width="900"/>
</p>
...
Example:
  ...
  Shift window:     Disruption 11 AM–2 PM within Ravi's 8 AM–10 PM  →  PASS
  Fixed rate:       ₹50/hr (Heavy Rain, Standard Shield)

  Payout = ₹50 × 3 = ₹150  →  auto-disbursed to UPI
```

**Why 60–70% income replacement, not 100%:** Parametric insurance by design does not fully replace income — this is basis risk, and it is intentional. A rider earns ~₹75/hr. Paying ₹50/hr (67% replacement) means honest workers are protected without the product becoming a profit opportunity. Full replacement creates a moral hazard where riders have no incentive to work through mild disruptions. The 60–70% band is the industry standard for parametric income protection.
...
| **Heavy Traffic Congestion** | **Speed ≥ 40% below historical baseline, sustained ≥ 45 min + order failure > 35%** | **Google Maps Traffic API + historical baseline model** | **₹40/hr** |
| **Internet Zone Blackout** | **Connectivity < 10% in zone for ≥ 30 min** | **Ookla / TRAI + device signal reports** | **₹50/hr** |

**Payout cap:** ₹150/day · ₹500/week (liquidity protection)
...
If all 3 match:
  ...
  →  Fixed payout = ₹40/hr × blocked_hours (Accident Blockspot rate, Standard Shield)
  →  SLA: 4 hours
```
...
Bengaluru's Electronic City flyover at 6 PM. Mumbai's Eastern Express Highway on a Friday. Chennai's GST Road after a rain shower clears. These are not rare events — Q-commerce riders face severe traffic congestion multiple times a week, trapping them for 45–90 minutes with zero orders possible. For a rider in a 2–3 km dark store radius, a blocked corridor isn't just slow — it's their entire operating zone gone.

**The actuarial challenge — paying for normal traffic is indefensible.** If Hustlr triggered every time Bengaluru's ORR slowed to 20 km/h, it would pay out every single evening and the pool would collapse in a week. The trigger is not "traffic is slow." The trigger is "traffic is significantly worse than it normally is at this exact time on this exact corridor."

**How Hustlr detects heavy traffic congestion:**

```
Step 1 — Build the historical baseline (per corridor, per 30-min time slot):
  Google Maps Traffic API historical data → rolling 90-day average speed
  per corridor segment per time slot

  Example:
    Electronic City Flyover, Mon–Fri 6:00–6:30 PM  →  avg 18 km/h  ±  4 km/h

Step 2 — Detect abnormal deviation in real time:
  Current speed < (baseline − 40%)  →  severe congestion flag
  Sustained for ≥ 45 minutes  →  confirms not a transient spike

  Example:
    Normal range:   14–22 km/h
    Trigger fires:  < 10.8 km/h  (40% below 18 km/h baseline)

Step 3 — Platform order failure rate corroboration:
  Order failure rate in affected zone > 35%  →  confirms riders cannot deliver
  (A slow road with riders still completing orders = not a covered event)

Trigger fires when all three conditions are met:
  Speed ≥ 40% below baseline  +  sustained ≥ 45 min  +  order failure > 35%
  →  AUTO_TRIGGER for workers whose dark store zone overlaps the corridor
```

**Why 40% below baseline, not a fixed speed threshold:**
Every corridor has its own normal congestion profile. A fixed threshold like "< 10 km/h" would trigger constantly on Bengaluru's ORR during rush hour — that is normal. The 40% deviation from each corridor's own rolling baseline is what identifies genuinely anomalous events. A corridor that normally runs at 18 km/h is disrupted at 10 km/h. A corridor that normally runs at 30 km/h is disrupted at 18 km/h. Same logic, different numbers. This is what makes the trigger actuarially defensible.

**Why 45 minutes minimum:**
A 10-minute jam that clears is not a shift-destroying event — riders can absorb that. 45 minutes sustained means the rider has missed at least one full delivery cycle and is genuinely losing income. It also filters out transient Traffic API measurement noise.

**Why order failure rate is required:**
Traffic API can show a slow road while riders on parallel routes are still delivering. The order failure rate check confirms the congestion is actually affecting rider operations in that zone, not just one road segment.

**City-specific corridor baselines (Phase 2):**

| City | High-Risk Corridor | Rush Hour Baseline | Trigger Threshold |
|---|---|---|---|
...
### Scenario — Heavy Traffic Congestion (Automated)

```
Date:     February 14, 2026 — Friday evening, 6:15 PM
Location: Electronic City Flyover, Bengaluru

Historical baseline (Mon–Fri 6:00–6:30 PM, 90-day avg):
  Average speed: 17 km/h  →  40% threshold: 10.2 km/h

Google Maps Traffic (real-time):
  6:15 PM — current speed: 4 km/h  →  76% below baseline  →  SEVERE CONGESTION FLAG

Sustained check:
  Speed < 10.2 km/h for 50 minutes continuously  →  ≥ 45 min threshold MET

Platform order failure rate:
  Electronic City zone: 52%  →  above 35% threshold  →  corroborated

All conditions met  →  AUTO_TRIGGER

Shift window: 6:15 PM within workers' active hours  →  PASS

Workers with Heavy Traffic coverage (Full Shield / Elite Shield):
  →  Fixed payout = ₹40/hr × verified congestion hours
  →  Disruption window: 6:15 PM – 7:45 PM = 1.5 hours
  →  Payout = ₹40 × 1.5 = ₹60
...
Calculation:
  ...
  Fixed hourly rate:  ₹50/hr (Standard Shield — 67% of ₹75/hr earnings)
  Disruption hours:   3
  Gross payout:       ₹150
...
Timeline:
  ...
  11:04 AM  →  ₹105 (70%) credited to Ravi's UPI immediately
  48 hrs    →  ₹45  (30%) released after review window

Total: ₹150.  Weekly premium paid: ₹49.  Net benefit: ₹101.
```
...
Workers on Standard Shield:
  →  Fixed payout = ₹50/hr × verified disruption hours (capped ₹150/day)
  →  Auto-claim pushed to ClaimCenter
  →  Zero worker action required
```
...
Outcome:
  Fixed rate:   ₹40/hr (Accident Blockspot, Standard Shield — 53% of ₹75/hr earnings)
...
Workers affected in Tambaram:
  →  Fixed payout = ₹50/hr × 2.5 hrs = ₹125 (Internet Blackout rate, Standard Shield)
  →  Auto-claim pushed to ClaimCenter
  →  Zero worker action required
...
Workers on Standard Shield in affected zones:
  ...
  →  Fixed payout = ₹50/hr × verified outage hours (capped ₹150/day)
  →  Auto-claim pushed to ClaimCenter
```
...
**Purpose:** Detects bandh, strike, curfew, and internet outage reports that weather and traffic APIs cannot capture. Polls every 15 minutes.
...
> Pricing is set at ≤ 2% of a typical rider's weekly income (₹5,200/week).
...
| **Standard Shield** ⭐ Most Popular | ₹49/wk | Rain, heat, pollution, app downtime | ~₹32 | 0.65 | Most city delivery workers |
| **Full Shield** | ₹79/wk | All disruption types incl. bandh + internet blackout | ~₹53 | 0.67 | Flood-zone and high-disruption workers |
| **Elite Shield** 🔥 Best Value | ₹109/wk | All types + heavy traffic + compound triggers + 10% cashback | ~₹60 | 0.55 | Bengaluru / Mumbai high-traffic zones |

**Affordability check:** Standard Shield at ₹49/week = 0.9% of a ₹5,200 weekly income. Even Elite Shield at ₹109/week = 2.1% — within the 2–3% threshold where insurance becomes a rational purchase for a gig worker.
...
| Heavy Traffic Congestion *(Phase 2)* | +₹15/wk | Traffic ≥ 40% below historical baseline for ≥ 45 min |
...
| Peak traffic congestion frequency | Google Maps Traffic API historical data |
...
- **Kolkata:** Highest bandh score in India + moderate flood + low peak traffic congestion
...
| Google Maps Traffic unavailable | Heavy Traffic trigger suspended; Accident trigger suspended; manual claims still accepted |
...
- Fixed hourly payout model (₹40–₹65/hr by trigger type, 60–70% income replacement) with daily ₹150 + weekly ₹500 caps
...
| **Daily worker cap** | ₹150/day | Limits per-worker exposure on high-disruption days |
| **Weekly worker cap** | ₹500/week | Liquidity protection during sustained disruption events (cyclone weeks) |
...
```
Weekly premium pool (10,000 workers × avg ₹49)  =  ₹4,90,000
80% available for payouts                         =  ₹3,92,000
20% held as liquid buffer                         =  ₹98,000
25% reserve (separate, not in pool)               =  ₹1,22,500

If a cyclone week triggers payouts above ₹3,92,000:
  → Reserve fund activates to cover overflow
  → If total losses exceed 4× pool (₹19,60,000):
...
| Average weekly premium (blended across tiers) | ₹49 |
| Weekly premium pool | ₹4,90,000 |
| Weekly payout cap (80% of pool) | ₹3,92,000 |
| Reserve fund (25% of premiums) | ₹1,22,500 accumulated/week |
| Loss ratio target | < 0.70 |
| Estimated real loss ratio (fixed payouts + caps) | ~55–65% |
